...
export async function initiateEvmEscrow(
  ...
  amountXRP: string,
  evmPublicWithdrawTimelock: number,
  evmCancellationTimelock: number,
  ...
  logProgress("--- [DEBUG] Starting initiateEvmEscrow ---");
  ...
  try {
    ...
    conditionData = generateConditionData(); //JSON.parse(execSync("node services/core/helpers/xrpl-condition-generator.js").toString());
    console.log("[DEBUG] Generated condition data:", conditionData);
  } catch (error: any) {
    logProgress(
      `[ERROR] Step 1 failed: Error generating condition data. Ensure scripts/helpers/xrpl-condition-generator.js exists and works. Error: ${error.message}`,
    );
  ...
  if (!RPC_URL) {
    console.error("[ERROR] NEXT_PUBLIC_EVM_RPC_URL environment variable is not set.");
    return null;
  }
  ...
  logProgress("[DEBUG] Step 2: Validating input parameters and configuration...");
  ...
  logProgress("[DEBUG] Input parameters and configuration validated successfully.");

  const provider = new ethers.JsonRpcProvider(RPC_URL);
  const wallet = new Wallet(DEPLOYER_PRIVATE_KEY, provider);
  logProgress("[DEBUG] Step 3: Interacting with Escrow Factory...");
  const escrowFactory = new ethers.Contract(ESCROW_FACTORY_ADDRESS, ESCROW_FACTORY_ABI, wallet);
  logProgress(`[DEBUG] EscrowFactory address: ${ESCROW_FACTORY_ADDRESS}`);
  ...
  logProgress("[DEBUG] Step 4: Approving WETH for EscrowFactory...");
  console.log(`[DEBUG] EscrowFactory address: ${ESCROW_FACTORY_ADDRESS}`);
  console.log(`[DEBUG] WETH token address: ${makerEVMTokenAddress}`);
  console.log(`[DEBUG] makerEVMAddress: ${makerEVMAddress}`);
  console.log(`[DEBUG] wethContract: ${wethContract}`);
  try {
    const approveTx = await wethContract.approve(ESCROW_FACTORY_ADDRESS, amountEVM);
    logProgress(`[DEBUG] WETH approval transaction sent. Hash: ${approveTx.hash}`);
    await approveTx.wait();
    logProgress("WETH approval successful!");
  } catch (error: any) {
    logProgress(`[ERROR] Step 4 failed: Error approving WETH: ${error}`);
    return null; // Return null on error
  }
  ...
  logProgress("[DEBUG] Step 5: Preparing Timelocks...");
  ...
  logProgress("Timelocks:");
  console.log("dstWithdrawalOffset (DeployedAt):", dstWithdrawalOffset);
  console.log("dstPublicWithdrawalOffset (EVM Public Withdrawal Timelock):", evmPublicWithdrawTimelock);
  console.log("dstCancellationOffset (EVM Cancellation Timelock):", evmCancellationTimelock);

  const timelocksPacked = packTimelocks(dstWithdrawalOffset, {
    dstWithdrawal: evmPublicWithdrawTimelock,
    dstCancellation: evmCancellationTimelock,
  });
  logProgress(`[DEBUG] Packed Timelocks(BigInt): ${timelocksPacked.toString()}`);
  ...
  logProgress(`DstImmutables:${dstImmutables}`);
  ...
  try {
    ...
    logProgress("[DEBUG] Step 6: Sending createDstEscrow transaction...");
    const tx = await escrowFactory.createDstEscrow(dstImmutables, srcCancellationTimestamp, {
      value: safetyDepositAmount,
    });
    await tx.wait();
    evmTxHash = tx.hash;
    console.log("[DEBUG] createDstEscrow transaction sent. Hash:", evmTxHash);
    logProgress(`[DEBUG] createDstEscrow transaction sent.Hash: ${evmTxHash}`);

    console.log("Cross-chain swap initiation successful!");
    logProgress("Cross-chain swap initiation successful!");
    ...
    const swapData = {
      ...
      evmTxHash: evmTxHash,
      evmTimelock: evmCancellationTimelock, // HTLC critical component
      creationTime: currentTime,
      evmPublicWithdrawTimelock: evmPublicWithdrawTimelock, // HTLC critical component
    ...
    logProgress(`Swap data stored successfully.UUID: ${uuid}`);
    ...
    const monitorData = {
      ...
      timelock: evmCancellationTimelock, // The timelock after which the EVM escrow can be cancelled
  ...
  logProgress("--- [DEBUG] initiateEvmEscrow finished. ---");
  // Return critical data for the next steps in the overall swap process
  if (uuid && evmTxHash) {
    return {
      uuid,
      evmTxHash,
      evmTimelock: evmCancellationTimelock,
      evmPublicWithdrawTimelock: evmPublicWithdrawTimelock,
